...
function ChooseRidersScreen({navigation}) {
    ...
    return (
        ...
        <ScrollView style={styles.container}>
...

            {
                online ? 
                riders?.map(rider => (
                    myridersID?.some(e => e.id === rider.id) ? null : <Add key={rider.id} ridername={rider.firstname + ' ' + rider.name}
                    credits={rider.credits} flag={flag(rider.country_id)} rider_id={rider.id} credits={rider.credits} total={credits} inTeam={false} r_u={null} teamRiders={myridersID?.length} />                    
                ))
                :
...
export default ChooseRidersScreen